...
def load_vgg(sess, vgg_path):
    ...
    vgg_keep_prob_name = 'keep_prob:0'
    ...
    keep_prob = graph.get_tensor_by_name(vgg_keep_prob_name)
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    conv_1x1_vgg_layer7_out = tf.layers.conv2d(vgg_layer7_out, num_classes, kernel_size=(1, 1), strides=(1, 1), padding='same',
        kernel_initializer=tf.truncated_normal_initializer(stddev=0.01), kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    layer8 = tf.layers.conv2d_transpose(conv_1x1_vgg_layer7_out, num_classes, kernel_size=(4, 4), strides=(2, 2), padding='same',
        kernel_initializer=tf.truncated_normal_initializer(stddev=0.01), kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    conv_1x1_vgg_layer4_out = tf.layers.conv2d(vgg_layer4_out, num_classes, kernel_size=(1, 1), strides=(1, 1), padding='same',
        kernel_initializer=tf.truncated_normal_initializer(stddev=0.01), kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    layer8_4 = tf.add(layer8, conv_1x1_vgg_layer4_out)

    layer9 = tf.layers.conv2d_transpose(layer8_4, num_classes, kernel_size=(4, 4), strides=(2, 2), padding='same',
        kernel_initializer=tf.truncated_normal_initializer(stddev=0.01), kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3)) 

    conv_1x1_layer3_out = tf.layers.conv2d(vgg_layer3_out, num_classes, kernel_size=(1, 1), strides=(1, 1), padding='same',
        kernel_initializer=tf.truncated_normal_initializer(stddev=0.01), kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    layer9_3 = tf.add(layer9, conv_1x1_layer3_out)

    out = tf.layers.conv2d_transpose(layer9_3, num_classes, kernel_size=(16, 16), strides=(8, 8), padding='same',
        kernel_initializer=tf.truncated_normal_initializer(stddev=0.01), kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3)) 
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
             correct_label, keep_prob_tensor, learning_rate_tensor, keep_prob, learning_rate):
    """
...
def run():
    ...
    with tf.Session() as sess:
        ...
        saver.save(sess, os.path.join(models_dir, 'vgg-fcn8'))
